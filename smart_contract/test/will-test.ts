...
describe("Will", async function () {
  ...
  async function deployContractFixture() {
    ...
    const [client, beneficiary] = await ethers.getSigners();
    ...
    return { will, client, beneficiary };
  }
  ...
  it("Should transfer assets to beneficiary", async () => {
    const { will, client, beneficiary } = await loadFixture(deployContractFixture);
    ...
    // const amount = clientBalance.sub(ethers.utils.parseEther(oneETH.toString()));
